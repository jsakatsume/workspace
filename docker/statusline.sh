#!/bin/bash
#
# Claude Code status line: model, context use, working directory, git branch,
# and the 5-hour / weekly usage limits.
#
# To show the usage limits it READS YOUR CLAUDE OAUTH TOKEN from
# $CLAUDE_CODE_OAUTH_TOKEN, the macOS Keychain, ~/.claude/.credentials.json, or
# the Linux keyring, and sends it to https://api.anthropic.com/api/oauth/usage.
# The response is cached in /tmp/claude/statusline-usage-cache.json. The token
# is used for nothing else and is never written to disk by this script.
#
# If you do not want that, drop "statusLine" from .claude/settings.json.
# Set STATUSLINE_TZ to choose the timezone of the clock (default: UTC).
set -f
...
# ── Timezone ────────────────────────────────────────────
STATUSLINE_TZ="${STATUSLINE_TZ:-UTC}"
...
format_epoch_time() {
    ...
    case "$style" in
        time)
            result=$(TZ="$STATUSLINE_TZ" date -j -r "$epoch" +"%l:%M%p" 2>/dev/null)
            [ -z "$result" ] && result=$(TZ="$STATUSLINE_TZ" date -d "@$epoch" +"%l:%M%P" 2>/dev/null)
            result=$(echo "$result" | sed 's/^ //; s/\.//g' | tr '[:upper:]' '[:lower:]')
            ;;
        datetime)
            result=$(TZ="$STATUSLINE_TZ" date -j -r "$epoch" +"%b %-d, %l:%M%p" 2>/dev/null)
            [ -z "$result" ] && result=$(TZ="$STATUSLINE_TZ" date -d "@$epoch" +"%b %-d, %l:%M%P" 2>/dev/null)
            result=$(echo "$result" | sed 's/  / /g; s/^ //; s/\.//g' | tr '[:upper:]' '[:lower:]')
            ;;
        *)
            result=$(TZ="$STATUSLINE_TZ" date -j -r "$epoch" +"%b %-d" 2>/dev/null)
            [ -z "$result" ] && result=$(TZ="$STATUSLINE_TZ" date -d "@$epoch" +"%b %-d" 2>/dev/null)
...
if ! $has_stdin_rates; then
    ...
    if $needs_refresh; then
        ...
        if [ -n "$token" ] && [ "$token" != "null" ]; then
            # The pinned User-Agent is what the usage endpoint expects; bump it
            # if the endpoint starts rejecting the request.
...
if [ "$extra_enabled" = "true" ] && [ -n "$usage_data" ]; then
    ...
    extra_reset=$(TZ="$STATUSLINE_TZ" date -v+1m -v1d +"%b %-d" 2>/dev/null | tr '[:upper:]' '[:lower:]')
    if [ -z "$extra_reset" ]; then
        extra_reset=$(TZ="$STATUSLINE_TZ" date -d "$(TZ="$STATUSLINE_TZ" date +%Y-%m-01) +1 month" +"%b %-d" 2>/dev/null | tr '[:upper:]' '[:lower:]')
    fi
